...
# ─────────────────────────────────────────
# FILE FILTER — acepta CSV, TXT, DAT y archivos sin extensión (IMS, etc.)
# ─────────────────────────────────────────
...
def read_flexible(file_obj, sep_val: str, col_sig: str | None = None, usecols=None):
    """
    Lee un CSV/TXT/DAT con o sin encabezado.
    Devuelve (DataFrame, nombre_columna_señal, tiene_header).
    """
    raw = file_obj.read()

    # Intento 1: con encabezado
    try:
        df = pd.read_csv(io.BytesIO(raw), sep=sep_val, engine="python",
                         dtype=np.float32, usecols=usecols)
        # Si la primera columna parece numérica fue leída como header → sin header
    ...
    # Intento 2: sin encabezado → genera nombres Canal_1, Canal_2 …
    df = pd.read_csv(io.BytesIO(raw), sep=sep_val, engine="python",
                     header=None, dtype=np.float32)
    df.columns = [f"Canal_{i+1}" for i in range(df.shape[1])]
    # Usa la primera columna de señal disponible si no se especificó
        ...
        for i, name in enumerate(valid_files):
            if i % 10 == 0:
                bar.progress((i + 1) / total,
                             text=f"Procesando {i+1}/{total} — {name.split('/')[-1]}")

            with z.open(name) as f:
                usecols = None if col_time == "Usar nombre del archivo" else None  # leemos todo y filtramos después
                try:
...
# ─────────────────────────────────────────
# PEEK SIDEBAR — detecta columnas del primer archivo
# ─────────────────────────────────────────
def peek_zip(uploaded_zip, sep_val: str):
    """Devuelve (df_peek, cols) del primer archivo válido del ZIP."""
    try:
...
def plotly_line(df, y_col, title, color, threshold=None, thr_label="Umbral"):
    ...
    fig.add_trace(go.Scatter(
        x=df["datetime"], y=df[y_col], mode="lines+markers", name=y_col,
    ...
    if threshold is not None:
        ...
        if mask.any():
            fig.add_trace(go.Scatter(
                x=df[mask]["datetime"], y=df[mask][y_col],
    ...
    fig.update_layout(
        ...
        xaxis=dict(gridcolor="#2A2D3E", title="Identificador / Tiempo",
                   type="category" if is_cat else "-"),
...
def make_chart_png(df, y_col, ylabel, color, threshold=None):
    ...
    is_cat = df["datetime"].dtype == "O"
    x_data = range(len(df)) if is_cat else df["datetime"]
    ...
    if threshold is not None:
        ...
        mask = df[y_col] > threshold
        if mask.any():
            ax.scatter(np.array(list(x_data))[mask], df.loc[mask, y_col],
    ...
    if is_cat:
        ...
        ax.set_xticks(ticks)
        ax.set_xticklabels([df["datetime"].iloc[t] for t in ticks],
...
def generate_pdf_report(df, thr_rms, thr_kurt, sigma_mult, baseline_n,
                         alarm_rms, alarm_kurt, client_name, equipo, ingeniero, fs_hz):
    buf = io.BytesIO()
    ...
    story.append(Paragraph(
        ...
        f"<b>{fs_hz/1000:.1f} kHz</b>, extrayendo indicadores estadísticos en dominio del tiempo.",
        S["body"]
    ))
    ...
    story.append(Paragraph(
        f"<i>Baseline: primeros {baseline_n} archivos · Umbral = media + {sigma_mult:.1f}σ</i>",
        S["body"]
    ))
    story.append(Spacer(1, 14))
    ...
    for label, y_col, ylabel, color, thr in [
        ...
        story.append(Spacer(1, 6))
        story.append(RLImage(make_chart_png(df, y_col, ylabel, color, thr),
...
with e2:
    with st.spinner("Compilando reporte técnico PDF..."):
        pdf_bytes = generate_pdf_report(
            ...
            client_name=client_name, equipo=equipo_name,
            ingeniero=ingeniero_nm, fs_hz=fs_hz
        )
    st.download_button(
